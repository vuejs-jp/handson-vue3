...
現在は商品のカードを表示しているだけですが、ヘッダーの情報が増えたり、フッターなどのコンテンツを足していくと `template` のコードはどんどん肥大化していきます。それだけでなく、例えば商品に複数写真を表示、個数によって表示金額の変更、といった機能を追加していくと `script` のコードも肥大化していきます。そうするとコードの見通しも悪くなり、メンテナンスも大変になってきます。そのような状況に陥らないため、Vue.js では機能の一部をモジュールとして切り出すことができるので、カードをコンポーネント化してみましょう。
...
## 一部をモジュールとして切り出す
モジュールとして切り出す時、どの範囲で切り出すか迷うかもしれません。そういった場合は、再利用可能という観点でも考えてみてもよいかもしれません。今回の場合、商品のカードの部分は、`v-for` の中で何度も呼ばれているので、この範囲で切り出してみるのが良さそうです。
...
切り出した `Card` コンポーネントを `import` して、`template` 内で呼び出しています。しかし現状エラーがでて動きません。作成した `Card` コンポーネントは `item` のデータを持っていないためです。そのため、`Card` コンポーネントでも `item` のデータを使えるように `props` として渡す必要があります。

### App.vue から値を受け取る準備をする
...
Vue.js では `emits` オプションが使えます。`emits` オプションは、子のコンポーネント内で親に発行できるイベントを定義できます。
...
子のコンポーネントの売り切れのボタンをクリックすると、該当の `soldOut` プロパティを変更し、カードが非表示になることが確認できました。
