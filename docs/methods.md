...
入荷ボタンをクリックすると `stockItem()` 関数によってリアクティブな値である `items` 配列の中身が変更されます。`items` 配列が変更されたことにより `template` が更新されるので、 `stockQuantity()` 関数が実行され、商品数が ３ から ４ に更新されます。
...
商品数には ３ が表示され、入荷ボタンをクリックすると ４ に更新されます。
...
`methods` オプションでも `computed` オプションでも最終的に同じ値を取得することが可能ですが、２ つのオプションの違いはどこにあるのでしょうか
